(() => {
  ...
  const buildEliminationMatches = (players, seeds) => {
    const matches = [];
    const bracketSize = 2 ** Math.ceil(Math.log2(players.length));
    const seedOrder = buildSeedOrder(bracketSize);
    const slots = seedOrder.map((seed) => (seed <= players.length ? players[seed - 1] : 'Bye'));
    let matchId = 1;
    for (let i = 0; i < slots.length; i += 2) {
      const player1 = slots[i];
      const player2 = slots[i + 1];
      const seed1 = player1 === 'Bye' ? null : seeds[player1];
      const seed2 = player2 === 'Bye' ? null : seeds[player2];
      if (player2 === 'Bye') {
        matches.push({
          id: matchId,
          round: 1,
  ...
  const buildSwissRound = ({ players, seeds, round, existingMatches, startId }) => {
    const standings = computeStandings({
      players,
      matches: existingMatches,
    });
    const scoreGroups = standings.reduce((acc, row) => {
    ...
    const playedPairs = new Set(
      existingMatches
        .filter((match) => match.player2 !== 'Bye')
        .map((match) => [match.player1, match.player2].sort().join('|')),
    );
    const pairings = [];
    let carry = [];
    sortedScores.forEach((score) => {
      const group = [...scoreGroups[score], ...carry];
      carry = [];
      group.sort((a, b) => seeds[a] - seeds[b]);
      const remaining = [...group];
      while (remaining.length > 1) {
        const player = remaining.shift();
        let opponentIndex = remaining.findIndex(
          (opponent) => !playedPairs.has([player, opponent].sort().join('|')),
        );
        if (opponentIndex === -1) {
          opponentIndex = 0;
        }
  ...
  const createMatches = (type, players, seeds) => {
    if (type === 'round robin') {
      const matches = [];
      let matchId = 1;
      let round = 1;
    ...
    return buildEliminationMatches(players, seeds);
  };
  ...
  const normalizeMatchIndexes = (tournament) => {
  ...
  const ensureNextRound = (tournament) => {
    ...
    if (winners.length < 2 && tournament.type !== 'swiss') {
      return;
    }
    ...
    const nextMatches =
      tournament.type === 'swiss'
        ? buildSwissRound({
            ...
            startId: tournament.matches.length + 1,
          })
        : winners.reduce((acc, winner, index) => {
            ...
            .map((pair, index) => {
              ...
              const player2 = pair[1] || 'Bye';
              const seed1 = tournament.seeds[player1];
              const seed2 = player2 === 'Bye' ? null : tournament.seeds[player2];
              const isBye = player2 === 'Bye';
              return {
                id: tournament.matches.length + 1 + index,
                round: nextRound,
                matchIndex: index + 1,
                player1,
                player2,
  ...
  const renderBracket = (tournament) => {
    ...
    if (tournament.type === 'round robin' || tournament.type === 'swiss') {
      bracketView.innerHTML =
        '<p class="tournaments__bracket-empty">Bracket view is available for elimination formats.</p>';
      return;
    }
    const rounds = tournament.matches.reduce((acc, match) => {
      acc[match.round] = acc[match.round] || [];
  ...
  const renderTournament = (slug) => {
    ...
    if (!tournament.seeds) {
      tournament.seeds = tournament.players.reduce((acc, player, index) => {
        acc[player] = index + 1;
        return acc;
      }, {});
      persistTournament(tournament);
  ...
  const setupForm = () => {
    ...
    form.addEventListener('submit', (event) => {
      ...
      const tournament = {
        ...
        type,
        players,
        seeds: players.reduce((acc, player, index) => {
          acc[player] = index + 1;
          return acc;
        }, {}),
        matches: [],
      ...
      tournament.matches = createMatches(type, players, tournament.seeds);
      ensureNextRound(tournament);
      persistTournament(tournament);
