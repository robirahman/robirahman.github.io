...
**Percentiles & Quartiles**: The pth percentile is the value below which p% of data falls. Q1 = 25th, Q2 = 50th (median), Q3 = 75th percentile.`,
    examples: [
      ...
      { label: 'Variance', formula: 'σ² = Σ(xᵢ − μ)² / n', explanation: 'Population variance. Sample variance divides by (n−1) for unbiased estimation (Bessel\'s correction).' },
      { label: 'IQR', formula: 'IQR = Q3 − Q1', explanation: 'Range of the middle 50% of data. Outliers often defined as values > Q3 + 1.5·IQR or < Q1 − 1.5·IQR.' },
    ],
    quizzes: [
      {
        type: 'mc',
...

  {
    ...
    icon: '🎲',
    concepts: ['sample space', 'event', 'probability axioms', 'complement', 'union', 'intersection', 'conditional probability', 'independence', 'multiplication rule'],
...
- Ignoring the sample space when computing probabilities`,
    examples: [
      ...
      { label: 'Conditional Probability', formula: 'P(A|B) = P(A ∩ B) / P(B)', explanation: 'Restricts the sample space to B, then measures A within it.' },
      { label: 'Independence Test', formula: 'P(A ∩ B) = P(A) · P(B)', explanation: 'If this holds, A and B are independent; knowing one doesn\'t change the other\'s probability.' },
    ],
    quizzes: [
      {
        type: 'mc',
...

  {
    ...
    icon: '🔄',
    concepts: ['prior', 'likelihood', 'posterior', 'marginal probability', 'Bayes theorem', 'Bayesian inference', 'frequentist inference', 'base rate'],
...
**Bayesian Inference in ML**: Naive Bayes, Gaussian Processes, Bayesian optimization, Bayesian neural networks. Priors act as regularization.`,
    examples: [
      { label: "Bayes' Theorem", formula: 'P(H|E) = P(E|H) · P(H) / P(E)', explanation: 'Posterior ∝ Likelihood × Prior. The normalizing constant P(E) ensures probabilities sum to 1.' },
      { label: 'Law of Total Probability', formula: 'P(E) = P(E|H)P(H) + P(E|Hᶜ)P(Hᶜ)', explanation: 'Marginalizes over all hypotheses to compute the overall probability of evidence.' },
    ],
    quizzes: [
      {
        type: 'mc',
      ...
      {
        type: 'mc',
        question: 'A disease affects 0.1% of the population. A test has 95% sensitivity and 95% specificity. You test positive. The probability you have the disease is approximately:',
      ...
      },
      {
        ...
        answer: 0,
        explanation: 'A prior encodes beliefs before data. Strong priors pull estimates toward the prior (like regularization pulling weights toward zero).',
      },
      {
        type: 'tf',
...

  {
    ...
    icon: '📈',
    concepts: ['normal distribution', 'binomial distribution', 'Poisson distribution', 't-distribution', 'chi-square distribution', 'F-distribution', 'parameters', 'PDF', 'CDF'],
...
**F-Distribution**: Ratio of two chi-square variables divided by their degrees of freedom. Used in ANOVA and comparing model variances.`,
    examples: [
      { label: 'Normal PDF', formula: 'f(x) = (1/σ√2π) · exp(−(x−μ)²/2σ²)', explanation: 'Symmetric bell curve. Fully described by μ (center) and σ (spread).' },
    ...
    quizzes: [
      ...
      },
      {
        ...
        answer: 0,
        explanation: 'n=100 independent trials, p=0.5 constant probability → Binomial(100, 0.5). (Approximately normal by CLT, but exactly binomial.)',
      },
      {
        type: 'mc',
...
- The theorem is about the mean; individual observations are not necessarily normal`,
    examples: [
      { label: 'CLT', formula: 'X̄ ~ N(μ, σ²/n) as n → ∞', explanation: 'The sample mean is approximately normally distributed with mean μ and variance σ²/n.' },
      { label: 'Standard Error', formula: 'SE(X̄) = σ / √n', explanation: 'Standard deviation of the sampling distribution. Halving SE requires quadrupling sample size.' },
...

  {
    ...
    icon: '🔬',
    concepts: ['null hypothesis', 'alternative hypothesis', 'test statistic', 'p-value', 'significance level', 'Type I error', 'Type II error', 'statistical power', 'one-tailed vs two-tailed'],
...
**Common Tests**: t-test (means), chi-square (categories), ANOVA (multiple means), Mann-Whitney U (non-parametric).`,
    examples: [
      { label: 'One-sample t-statistic', formula: 't = (X̄ − μ₀) / (s/√n)', explanation: 'Tests if sample mean differs from hypothesized μ₀. Degrees of freedom = n − 1.' },
      { label: 'Two-sample t-statistic', formula: 't = (X̄₁ − X̄₂) / SE(X̄₁ − X̄₂)', explanation: 'Tests if two group means differ. Welch\'s t-test doesn\'t assume equal variances.' },
      { label: 'Power', formula: 'Power = 1 − β = P(reject H₀ | H₀ false)', explanation: 'Determined by effect size, sample size, α, and data variance. Power ≥ 0.8 is typical target.' },
    ],
    quizzes: [
      {
        type: 'mc',
...
**Relationship to Hypothesis Testing**: A 95% CI is equivalent to a two-tailed test at α = 0.05. If the CI doesn't include the null value, you'd reject H₀.`,
    examples: [
      { label: '95% CI for mean (known σ)', formula: 'X̄ ± 1.96 · σ/√n', explanation: '1.96 is the z* for 95% confidence (middle 95% of normal distribution).' },
      ...
      { label: 'CI width', formula: 'Width = 2 · z* · σ/√n', explanation: 'To halve the width, quadruple n. Higher confidence level → wider interval.' },
    ],
    quizzes: [
      {
        type: 'mc',
        ...
        answer: 0,
        explanation: 'In the frequentist view, the true mean is fixed. The CI is a random interval that captures it 95% of the time in repeated sampling.',
      },
      {
        type: 'mc',
...

  {
    ...
    icon: '🔗',
    concepts: ['Pearson correlation', 'Spearman correlation', 'covariance', 'correlation vs causation', 'spurious correlation', 'Simpson\'s paradox', 'partial correlation'],
...
**Partial Correlation**: Correlation between X and Y controlling for variable Z. Removes Z's influence on both X and Y before computing correlation.`,
    examples: [
      ...
      { label: 'Covariance', formula: 'Cov(X,Y) = Σ(xᵢ − x̄)(yᵢ − ȳ) / (n−1)', explanation: 'Positive = variables move together; negative = opposite directions; 0 = uncorrelated. Scale-dependent.' },
      { label: 'Spearman ρ', formula: 'ρ = 1 − 6Σdᵢ² / n(n²−1)', explanation: 'dᵢ = rank difference between paired observations. Works for monotone non-linear relationships.' },
    ],
    quizzes: [
      {
        type: 'mc',
...

  {
    ...
    icon: '📉',
    concepts: ['OLS', 'regression coefficients', 'residuals', 'R²', 'adjusted R²', 'regression assumptions', 'heteroscedasticity', 'multicollinearity', 'VIF'],
...
- **Cook's distance**: identifies influential observations`,
    examples: [
      { label: 'OLS Estimator', formula: 'β̂ = (XᵀX)⁻¹Xᵀy', explanation: 'Matrix form. Minimizes sum of squared residuals. Requires XᵀX to be invertible (no perfect multicollinearity).' },
      { label: 'R²', formula: 'R² = 1 − SS_res/SS_tot = 1 − Σ(yᵢ−ŷᵢ)²/Σ(yᵢ−ȳ)²', explanation: 'Proportion of variance explained. Adding any predictor increases R² even if it\'s useless.' },
      { label: 'Adjusted R²', formula: 'R²_adj = 1 − (1−R²)(n−1)/(n−p−1)', explanation: 'Penalizes extra predictors p. Decreases if useless predictors are added.' },
    ],
    quizzes: [
      {
        type: 'mc',
      ...
      },
      {
        ...
        options: ['Expected — R² always increases with more variables', 'Suspicious — noise should not improve fit', 'Impossible', 'Good model improvement'],
        answer: 0,
      ...
      },
      {
        ...
        answer: 0,
        explanation: 'Fan shape (residuals spread increasing with fitted values) = non-constant variance = heteroscedasticity. Violates OLS assumption 4.',
      },
      {
        type: 'tf',
        question: 'A high VIF (Variance Inflation Factor) for a predictor indicates it is correlated with other predictors.',
        answer: true,
        explanation: 'VIF measures how much a predictor\'s variance is inflated by collinearity with others. VIF > 10 suggests problematic multicollinearity.',
      },
    ],
  },

  {
    ...
    icon: '🧪',
    concepts: ['A/B test', 'randomization', 'control group', 'treatment group', 'MDE', 'sample size calculation', 'multiple testing', 'Bonferroni correction', 'novelty effect', 'CUPED'],
...
**CUPED** (Controlled-experiment Using Pre-Experiment Data): reduces variance by controlling for pre-experiment metric, increasing power without more users.`,
    examples: [
      { label: 'MDE', formula: 'MDE = (z_α/2 + z_β) · √(2 · p̄(1−p̄)/n)', explanation: 'Smallest detectable difference. Smaller MDE → larger required n. Plan this before running the experiment.' },
      { label: 'Bonferroni Correction', formula: 'α_corrected = α / m', explanation: 'm = number of tests. Very conservative for large m. Reduces power substantially.' },
      { label: 'Relative Uplift', formula: 'Uplift = (p_B − p_A) / p_A', explanation: 'Often more interpretable than absolute difference for business metrics.' },
    ],
    quizzes: [
      {
        type: 'mc',
