...
const ALGORITHM_DATA = [
  // ── SUPERVISED LEARNING ──────────────────────────────────────────────────
  {
    ...
    fullDef:
      "Linear regression fits a hyperplane y = β₀ + β₁x₁ + … + βₙxₙ by minimizing the sum of squared residuals (OLS). It assumes a linear relationship between inputs and output, homoscedastic errors, and no multicollinearity. Ridge and Lasso add L2/L1 regularization to shrink coefficients.",
...
print(model.coef_, model.intercept_)`,
  ...
  },
  {
    ...
    fullDef:
      "Despite the name, logistic regression is a classification algorithm. It estimates P(y=1|x) = σ(βᵀx) where σ is the sigmoid function. Trained by maximizing log-likelihood. Extends to multi-class via one-vs-rest or softmax (multinomial). L1/L2 regularization prevents overfitting.",
    whenToUse:
      "Binary or multi-class classification with interpretable probabilities; medical risk scoring; when you need calibrated probability outputs.",
...
proba = model.predict_proba(X_test)[:, 1]`,
  ...
  },
  {
    ...
    fullDef:
      "A decision tree partitions the input space by greedily choosing splits that minimize Gini impurity (classification) or MSE (regression). Trees are fully grown to low bias but high variance; pruning (max_depth, min_samples_leaf) controls overfitting. CART is the most common algorithm.",
    whenToUse:
      "Quick interpretable baseline; feature importance estimation; when data has non-linear/interaction effects and interpretability matters.",
...
importances = model.feature_importances_`,
  ...
  },
  {
    ...
    fullDef:
      "k-NN is a lazy, non-parametric algorithm — it stores the entire training set and makes predictions at query time by finding the k nearest points by distance (Euclidean, Manhattan, Minkowski). No training phase; all computation happens at inference. k controls the bias-variance tradeoff: small k → low bias, high variance.",
    ...
    tradeoffs:
      "Slow inference on large datasets (O(n)); requires feature scaling; sensitive to irrelevant features and high dimensionality (curse of dimensionality).",
...
preds = model.predict(X_test)`,
  ...
  },
  {
    ...
    fullDef:
      "Naive Bayes computes P(class|features) ∝ P(class) × ∏P(featureᵢ|class) using the naive conditional independence assumption. Gaussian NB assumes normal features; Multinomial NB suits word counts; Bernoulli NB suits binary features. Despite the simplifying assumption, NB works well in practice for text.",
...
model.fit(X_train, y_train, eval_set=[(X_val, y_val)], early_stopping_rounds=20)`,
  ...
  },
  {
    ...
    fullDef:
      "Regularization modifies the training objective to penalize large coefficients. **L2 (Ridge)**: adds λΣβᵢ² — shrinks all coefficients toward zero but rarely to exactly zero; good when many features contribute. **L1 (Lasso)**: adds λΣ|βᵢ| — produces sparse solutions (some coefficients exactly zero) enabling feature selection. **ElasticNet**: combines L1 and L2 penalties (α·L1 + (1-α)·L2), balancing sparsity and grouping correlated features. The regularization strength λ (or C = 1/λ in sklearn) is tuned via cross-validation.",
    whenToUse:
      "High-dimensional data where overfitting is likely; Lasso for automatic feature selection; Ridge when you expect most features to contribute; ElasticNet when features are correlated.",
    tradeoffs:
      "Requires hyperparameter tuning (λ); L1 solutions are non-unique with correlated features; Ridge does not perform feature selection; both require feature scaling for fair penalization.",
...
cv_score = cross_val_score(enet, X, y, cv=5).mean()`,
  ...
  // ── UNSUPERVISED LEARNING ─────────────────────────────────────────────────
  {
    ...
    fullDef:
      "K-Means initializes k centroids randomly (or with k-means++ for better starts), assigns each point to the nearest centroid (Euclidean), then recomputes centroids as cluster means. Iterates until convergence. Minimizes within-cluster sum of squares (WCSS). The elbow method and silhouette score help choose k.",
    whenToUse: "Customer segmentation; document clustering; image quantization; as preprocessing for other algorithms.",
    tradeoffs:
      "Must specify k; sensitive to initialization and outliers; assumes spherical clusters of similar size; struggles with non-convex shapes.",
...
centroids = model.cluster_centers_`,
  ...
  },
  {
    ...
    fullDef:
      "Agglomerative (bottom-up) HC starts with each point as its own cluster and merges the closest pair iteratively. Linkage criteria (single, complete, average, Ward) define inter-cluster distance. The dendrogram can be cut at any level to produce any number of clusters. Divisive (top-down) is rarer.",
...
labels = model.fit_predict(X)`,
  ...
  },
  {
    ...
    fullDef:
      "DBSCAN (Density-Based Spatial Clustering of Applications with Noise) classifies points as core (≥ min_samples neighbors within ε), border, or noise. Clusters are dense regions separated by low-density areas. Does not require specifying k. Robust to outliers. Key params: ε (neighborhood radius) and min_samples.",
...
outliers = X[labels == -1]`,
  ...
  },
  {
    ...
    fullDef:
      "PCA finds orthogonal directions (principal components) that maximize explained variance. Computed via eigendecomposition of the covariance matrix or SVD of the data matrix. The first k components capture most variance. Used for visualization, noise reduction, and speeding up downstream models. Requires feature scaling first.",
...
print(pca.explained_variance_ratio_)`,
  ...
  },
  {
    ...
    fullDef:
      "t-SNE (t-distributed Stochastic Neighbor Embedding) models pairwise similarities as probabilities in high-D and low-D space, minimizing KL divergence between them using a t-distribution in 2D (to avoid crowding). Excellent for visualizing clusters. Perplexity (5–50) controls neighborhood size. Results are not reproducible without a fixed seed and vary with perplexity.",
...
class Autoencoder(nn.Module):
  ...
  // ── MODEL EVALUATION ──────────────────────────────────────────────────────
  {
    ...
    fullDef:
      "Accuracy = (TP + TN) / (TP + TN + FP + FN). Simple and intuitive, but misleading for imbalanced datasets (e.g., 99% negative class → 99% accuracy by always predicting negative). Always report alongside class distribution. Suitable when classes are balanced.",
...
fpr, tpr, thresholds = roc_curve(y_true, y_scores)`,
  ...
  },
  {
    ...
    fullDef:
      "RMSE = √(Σ(yᵢ - ŷᵢ)²/n) — same units as target, penalizes large errors. MAE = Σ|yᵢ - ŷᵢ|/n — more robust to outliers. R² = 1 - SS_res/SS_tot — fraction of variance explained; can be negative for very poor models. Adjusted R² penalizes extra features. RMSE is scale-dependent; use relative metrics (MAPE) to compare across targets.",
...
ll = log_loss(y_true, y_prob)  # y_prob: predicted probabilities`,
  ...
  },
  {
    ...
    tradeoffs:
      "You cannot simultaneously minimize both bias and variance with a single model; cross-validation estimates the tradeoff empirically; regularization shifts the optimal point.",
...
print(study.best_params)`,
  ...
  },
  {
    ...
    category: "evaluation",
    shortDef:
      "A model is calibrated if predicted probabilities match observed frequencies — e.g., 70% confident predictions are correct 70% of the time.",
    fullDef:
      "A well-calibrated classifier outputs P(y=1|x=p) ≈ p for all p. **Reliability diagram (calibration plot)**: bins predictions by confidence; plots mean predicted probability vs. observed frequency. Perfectly calibrated → points on the diagonal. **Brier Score**: mean squared error between probabilities and true labels (0=perfect, 1=worst). Many models are miscalibrated: tree ensembles (RF, GBM) tend to be overconfident; logistic regression is often well-calibrated; SVMs and naive Bayes are typically poorly calibrated. **Calibration methods**: Platt scaling (sigmoid fit to outputs), isotonic regression (non-parametric monotonic fit).",
    whenToUse:
      "Decision-making under uncertainty (medical diagnosis, credit scoring, weather forecasting); when using model probabilities for downstream decisions; after applying threshold adjustments.",
    tradeoffs:
      "Calibration and discrimination (AUC) are independent — a model can rank well but be poorly calibrated; calibration on small test sets is noisy; recalibration may slightly reduce discrimination.",
...
plt.ylabel('Fraction of positives')`,
  ...
  },
  {
    ...
    tradeoffs:
      "SMOTE can generate unrealistic samples; undersampling loses information; class weights don't help with test-time threshold; no single strategy dominates — evaluate on appropriate metric.",
...
print(classification_report(y_test, preds))`,
  ...
  // ── DEEP LEARNING ─────────────────────────────────────────────────────────
  {
    ...
    fullDef:
      "A perceptron computes ŷ = f(wᵀx + b) where f is an activation function. Stacking perceptrons in layers creates a feedforward neural network (MLP). With enough layers and units, MLPs are universal approximators. Training: forward pass computes loss; backward pass computes gradients via backpropagation; optimizer updates weights. Batch normalization and dropout stabilize training.",
...
model = nn.Sequential(
  ...
  },
  {
    ...
    fullDef:
      "Without activation functions, stacked linear layers collapse to a single linear transformation. ReLU(x) = max(0, x): fast, avoids vanishing gradients, most common. Sigmoid: squashes to (0,1), used for output probability in binary tasks; vanishing gradients in deep nets. Tanh: (−1, 1), zero-centered. Leaky ReLU, GELU, Swish address dying ReLU. Softmax normalizes outputs to a probability distribution (multi-class output).",
...
cnn = nn.Sequential(
  ...
  },
  {
    ...
    tradeoffs:
      "Sequential computation → slow to parallelize; LSTMs still struggle with very long dependencies; superseded by Transformers for many tasks.",
    codeExample: `import torch.nn as nn
...
output, (hn, cn) = lstm(x)  # x: (batch, seq_len, input_size)`,
  ...
  },
  {
    ...
    fullDef:
      "Transformers replace recurrence with self-attention: each token attends to all others via Q, K, V matrices (Attention(Q,K,V) = softmax(QKᵀ/√d)V). Multi-head attention runs multiple attention heads in parallel. Positional encodings inject order information. Encoder-decoder (T5), encoder-only (BERT), and decoder-only (GPT) variants exist. Scales remarkably well with data and compute.",
